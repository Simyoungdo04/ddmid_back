...
public class TmapService {
	...
	private TmapRouteDto parseRoute(JsonNode response) {
		if (response == null) {
			throw new IllegalStateException("Tmap 응답을 받지 못했습니다.");
		}
		JsonNode features = response.path("features");
		if (!features.isArray() || features.isEmpty()) {
			throw new NoRouteFoundException("두 지점을 잇는 도보 경로를 찾지 못했습니다.");
		}
		...
		if (points.isEmpty()) {
			throw new NoRouteFoundException("도보 경로 좌표를 받지 못했습니다.");
		}
